...
if __name__ == '__main__':
    ...
    audPts = SAMP_FREQ * SAMP_LEN_MS // 1000

    i16AudBuf = np.zeros(audPts, dtype="int16")
    f64AudBuf = np.zeros(audPts, dtype="double")

    freqAmp = [1.0, 0, 0.52, 0, 0.25, 0, 0.12, 0, 0.032]
    
    for i in range(16000):
        val = 0
        for (j, amp) in enumerate(freqAmp):
            val += np.sin(M_2PI / SAMP_FREQ * 4 * i * (j + 1)) * amp
        f64AudBuf[i] = val
    f64AudBuf = f64AudBuf / f64AudBuf.max() / 1.000001 / 4.0
    
    if hasPlot:
        plt.plot(np.arange(len(f64AudBuf)), f64AudBuf)
        plt.show()

    i16AudBuf = np.asarray(f64AudBuf * 32768, dtype="int16")
    
    kws = KWS_MFCC(i16AudBuf)

    mfccFeatures = kws.extract_features()

    u8MfccFeatures = kws.extract_uint8_features()

    print(u8MfccFeatures)

    if hasPlot:
        f, ax = plt.subplots(figsize=(10, 3))        
        ax.set_title('FP32 MFCC features')
        ax = seaborn.heatmap(mfccFeatures.reshape(kws.NUM_FRAMES, kws.NUM_MFCC_COEFFS).transpose([1,0]))
        plt.show()
        ax.set_title('UINT8 MFCC features')
        ax = seaborn.heatmap(u8MfccFeatures.reshape(kws.NUM_FRAMES, kws.NUM_MFCC_COEFFS).transpose([1,0]))
        plt.show()
